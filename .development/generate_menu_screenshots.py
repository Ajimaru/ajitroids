    ...
    for module in find_modul_modules():
        for _, cls in inspect.getmembers(module, inspect.isclass):
            ...
            try:
                ...
                name = cls.__name__
                if (has_draw and (has_update or has_activate)) or name.endswith(("Menu", "Screen", "Dashboard", "Viewer", "Display")):
                    classes[name] = cls
            except Exception:
                continue
    return classes
...
def build_hierarchy_and_render(classes):
    ...
    def render_node(name, path_parts):
        inst = instances.get(name)
        dirpath = os.path.join(LOG_DIR, *path_parts)
        os.makedirs(dirpath, exist_ok=True)
        out = os.path.join(dirpath, f"{name}.png")
        try:
        ...
        for child in sorted(children.get(name, [])):
            render_node(child, path_parts + [child])

    # find roots (no parent)
    roots = [n for n in classes if n not in parents]
    for root in sorted(roots):
        render_node(root, [root])

    # render any remaining unrendered classes into .logs/misc/
    misc = os.path.join(LOG_DIR, "misc")
    os.makedirs(misc, exist_ok=True)
    for name in classes:
        if name in rendered:
            continue
        inst = instances.get(name)
        out = os.path.join(misc, f"{name}.png")
        try:
            surface = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT))
            surface.fill((0, 0, 0))
            try:
                if inst:
                    inst.activate()
                    inst.draw(surface)
            except (AttributeError, TypeError, pygame.error, OSError) as e:
                logger.warning("Draw failed for %s: %s", name, e)
            pygame.image.save(surface, out)
            logger.info("Wrote %s", out)
        except (pygame.error, OSError) as e:
            logger.exception("Failed rendering %s into misc: %s", name, e)
