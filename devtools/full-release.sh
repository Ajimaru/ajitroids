...
if ! command -v pytest >/dev/null 2>&1; then
  ...
  if ! pytest --cov --cov-report=html:"${SCRIPT_DIR}/htmlcov" --import-mode=importlib; then
...
if [ -n "$LAST_TAG" ]; then
  ...
  if [[ "$REMOTE_URL" =~ github\.com[/:]([^/]+)/([^/.]+)(\.git)?$ ]]; then
    ...
    REPO="${BASH_REMATCH[2]}"
    DIFF_LINK="https://github.com/$USERNAME/$REPO/compare/$LAST_TAG...$NEW_VERSION"
  else
    DIFF_LINK=""
    color_msg "$YELLOW" "[WARNING] Could not parse GitHub URL. Comparison link will be skipped."
  fi
  echo "[Full Changelog](https://github.com/$USERNAME/$REPO/compare/$LAST_TAG...$NEW_VERSION)" >> "$CHANGELOG_FILE"
  [ -n "$DIFF_LINK" ] && log "Added comparison link: $DIFF_LINK"
fi
color_msg "$CYAN" "[CHANGELOG] Updated: $CHANGELOG_FILE"
log "Changelog updated: $NEW_VERSION"

git add -f "$VERSION_FILE" "$CHANGELOG_FILE"
git commit -m "chore(release): bump to $NEW_VERSION"
git tag "$NEW_VERSION"
color_msg "$GREEN" "[OK] Version bumped to $NEW_VERSION"
log "Version bumped: $NEW_VERSION"
