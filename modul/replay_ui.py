...
from modul.constants import MENU_TITLE_COLOR, MENU_SELECTED_COLOR, MENU_UNSELECTED_COLOR
...
class ReplayListMenu:
        ...
        for event in events:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    self.selected_index = (self.selected_index + 1) % len(self.replays)
                elif event.key in (pygame.K_RETURN, pygame.K_SPACE) and len(self.replays) > 0:
                    return {"action": "play_replay", "filepath": self.replays[self.selected_index]['filepath']}
    ...
    def draw(self, screen):
        ...
        # Title
        title_surf = self.title_font.render("REPLAY BROWSER", True, pygame.Color(MENU_TITLE_COLOR))
        title_rect = title_surf.get_rect(center=(SCREEN_WIDTH / 2, 60))
        screen.blit(title_surf, title_rect)
        
        if not self.replays:
            # No replays message
            no_replays_surf = self.text_font.render("No replays found", True, (200, 200, 200))
            no_replays_rect = no_replays_surf.get_rect(center=(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2))
            ...
            for i, replay in enumerate(self.replays):
                ...
                info_text = f"{date_str} | Score: {score:,} | Level: {level} | {difficulty} | {duration_str}"
                ...
                text_surf = self.text_font.render(prefix + info_text, True, color)
        ...
        for instruction in instructions:
            instr_surf = self.small_font.render(instruction, True, (150, 150, 150))
...
class ReplayViewer:
    ...
    def draw_hud(self, screen):
        ...
        status_color = (255, 200, 0) if self.replay_player.paused else (0, 255, 0)
        ...
        # Background
        pygame.draw.rect(screen, (50, 50, 50), (bar_x, bar_y_pos, bar_width, 10))
        
        # Progress
        fill_width = (progress / 100.0) * bar_width
        pygame.draw.rect(screen, (0, 200, 255), (bar_x, bar_y_pos, fill_width, 10))
        ...
        controls_rect = controls_surf.get_rect(center=(SCREEN_WIDTH / 2, bar_y + 65))
