...
class PowerUp(CircleShape):
    def __init__(self, x, y, powerup_type=None):
        ...
        self.type = powerup_type if powerup_type else random.choice(
            POWERUP_TYPES)
    ...
    def draw(self, screen):
        ...
        if self.lifetime <= 3.0:
            ...
            pulse_scale = 0.7 + 0.3 * \
                abs(math.sin(pulse_frequency * pygame.time.get_ticks() * 0.001))
        ...
        if self.type == "shield":
            pygame.draw.circle(screen, self.color,
                               self.position, draw_radius * 0.6, 1)
            ...
            for angle in [-30, 0, 30]:
                ...
                for i in range(5):
                    end_point = start_point + \
                        pygame.Vector2(0, -segment_length).rotate(angle_offset)
                    pygame.draw.line(screen, self.color,
                                     start_point, end_point, 2)
                    angle_offset += 5 if angle < 0 else (-5 if angle >
                                                         0 else 0)
            ...
            for angle in [30, 90, 150, 210, 270, 330]:
                ...
                for i in range(segments):
                    ...
                    offset_dir = pygame.Vector2(math.cos(math.radians(
                        offset_angle)), math.sin(math.radians(offset_angle)))

                    next_point = points[-1] + \
                        offset_dir * (distance / segments)
            ...
            rect = pygame.Rect(
                self.position.x - draw_radius * 0.25, self.position.y -
                draw_radius * 0.6, draw_radius * 0.5, draw_radius * 1.2
            )
            ...
            for i in range(3):
                ...
                pygame.draw.line(
                    screen, self.color, (self.position.x - draw_radius *
                                         0.25, y), (self.position.x + draw_radius * 0.25, y), 1
                )
            ...
            head_points = []
            head_points.append(
                self.position - pygame.Vector2(0, draw_radius * 0.6))
            head_points.append(
                self.position - pygame.Vector2(draw_radius * 0.3, draw_radius * 0.3))
            head_points.append(
                self.position - pygame.Vector2(-draw_radius * 0.3, draw_radius * 0.3))
            pygame.draw.polygon(screen, self.color, head_points, 1)

            wing_l = []
            wing_l.append(self.position +
                          pygame.Vector2(-draw_radius * 0.1, 0))
            wing_l.append(
                self.position + pygame.Vector2(-draw_radius * 0.4, draw_radius * 0.3))
            wing_l.append(
                self.position + pygame.Vector2(-draw_radius * 0.1, draw_radius * 0.3))
            pygame.draw.polygon(screen, self.color, wing_l, 1)

            wing_r = []
            wing_r.append(self.position + pygame.Vector2(draw_radius * 0.1, 0))
            wing_r.append(self.position +
                          pygame.Vector2(draw_radius * 0.4, draw_radius * 0.3))
            wing_r.append(self.position +
                          pygame.Vector2(draw_radius * 0.1, draw_radius * 0.3))
